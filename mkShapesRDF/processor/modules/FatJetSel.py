...
#  circa 120 di utils.py in JMECalculator, tau1, tau2
...
class FatJetSel(Module):
    ...
    # CleanJet not fat, CleanJet-CleanFatJet distance 
        ...
        df = df.Define("CleanJetNotFat_idx", "CleanJet_idx[isCleanJetNotFat]")
        ...
        variables = ["jetId", "pt", "eta", "mass", "msoftdrop"]
        for var in variables:
            df = df.Redefine(
                f"CleanFatJet_{var}", f"CleanFatJet_{var}[CleanFatJetMask]"
            )
        df = df.DropColumns("CleanFatJetMask")
        df = df.DropColumns("CleanJetNotFat_idx")
        df = df.DropColumns("CleanJetNotFat_deltaR")

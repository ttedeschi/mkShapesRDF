...
class SearchFiles:
        def __init__ (self):
                # cache result of `glob.glob` and `xrdfs ... ls ...`
        ...
        def searchFiles(self, folder, searchString, xrootd=False):
                ...
                if len(self.cached_list_of_files.get(folder, [])) == 0:
                        ...
                        if xrootd:
                                ...
                                proc = subprocess.Popen(f'xrdfs {redirector} ls {folder}', shell=True, stdout=subprocess.PIPE)
                                listOfFiles = proc.communicate()[0].decode('utf-8').split('\n')
                                #print(len(listOfFiles), listOfFiles[0])
                ...
                _files = list(filter(lambda k: fnmatch.fnmatch(k, folder + searchString), listOfFiles))
                if xrootd:

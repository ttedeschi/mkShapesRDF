...
def CombineBaseW(samples, proc, samplelist):
    ...
    newbaseW = str(xsDB[samplelist[0]] *1000 / df.Sum("genEventSumw").GetValue())
    #print(newbaseW)
...
samples['DY'] = {
    'name': files,
    'weight': mcCommonWeight + '*( !(Sum(PhotonGen_isPrompt==1 && PhotonGen_pt>15 && abs(PhotonGen_eta)<2.6) > 0)) * DY_weight',
    #'weight': mcCommonWeight,
    'FilesPerJob': 10,
#    'subsamples': {
#      'hardJets'  : 'hardJets',
...
samples['top'] = {
    'name': files,
    'weight': mcCommonWeight + '*top_weight',
    'FilesPerJob': 15,
}
